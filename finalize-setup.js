...
// Descargar y guardar la imagen del abogado
const abogadoUrl = 'https://i.postimg.cc/G3Yz1hBZ/Adobe-Express-file.png';
const abogadoLocalPath = path.join(process.cwd(), 'public', 'images', 'guillermo-nolivos.png');

// Actualizar el archivo del componente ParallaxHero para usar la imagen local
const updateParallaxImage = async () => {
  try {
    // Guardar la imagen localmente
    await saveImage(abogadoUrl, abogadoLocalPath);
    ...
    if (fs.existsSync(parallaxPath)) {
      ...
      content = content.replace(
        /src="https:\/\/i\.postimg\.cc\/G3Yz1hBZ\/Adobe-Express-file\.png"/,
      ...
      console.log('✅ Actualizado ParallaxHero.tsx con la imagen local');
    } else {
      console.log('⚠️ No se encontró el archivo ParallaxHero.tsx');
    }
  } catch (error) {
    console.error('⚠️ Error al actualizar la imagen:', error.message);
  }
};
